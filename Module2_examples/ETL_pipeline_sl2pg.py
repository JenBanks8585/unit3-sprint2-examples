...
def connect_to_pg(pg_dbname, pg_user, pg_password, pg_host, extraction_db='rpg_db.sqlite3'):
    ...
    pg_conn = psycopg2.connect(
        dbname=pg_dbname, user=pg_user, password=pg_password, host=pg_host)  # postgreSQL db
    return sl_conn, pg_conn


def execute_query(curs, query):
    # executes query depending on the cursor we have passed in
...
if __name__ == "__main__":
    ...
    # creating table in pg db
    execute_query(pg_curs, create_pg_character_table)
