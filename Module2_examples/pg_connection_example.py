...
if __name__ == "__main__":
    ...
    # this will give an error if we run twice, how can we fix? (try and execpt? Special SQL command?)
    execute_query(curs, create_table_statement)
    execute_query(curs, insert_statement)
    conn.commit()
    # checks to make sure we have the table by printing contents
    print(execute_query(curs, table_check))
